local Scanner = require 'lib.lustache.scanner'
local Context = require 'lib.lustache.context'
...
local function compile_tokens(tokens, originalTemplate)
    ...
    local function render(ctx, rnd)
        ...
        for i, token in ipairs(tokens) do
            local t = token.type
            buf[#buf + 1] =
                t == '?' and rnd:_conditional(token, ctx, subrender(i, token.tokens)) or
                t == '#' and rnd:_section(token, ctx, subrender(i, token.tokens), originalTemplate) or
                t == '^' and rnd:_inverted(token.value, ctx, subrender(i, token.tokens)) or
