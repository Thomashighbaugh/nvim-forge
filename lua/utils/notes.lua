-- Modified form of https://github.com/jellydn/my-note.nvim
...
M.config = {
    ...
    notes_dir = "~/.todos", -- Directory to store note files
    files = {
        global = "global-todos.md", -- Name of the global note file
        cwd = function()
...
local check_note_file = function(file)
    ...
    if
        vim.tbl_isempty(
            vim.fs.find(file, { type = "file", path = M.my_note_cache_dir })
        )
    then
...
M.setup = function(config)
    ...
    -- Define the command
    vim.api.nvim_create_user_command("ProjectTODOs", function(opts)
