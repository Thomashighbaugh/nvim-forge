...
local default_config = require('neovim.lsp.providers.defaults')
...
lsp_installer.on_server_ready(
    function(server)
        ...
        -- set up default neovim options
        if server.name == 'tsserver' then
            opts = utils.merge(opts, require('neovim.lsp.providers.tsserver'))
        elseif server.name == 'jsonls' then
            opts = utils.merge(opts, require('neovim.lsp.providers.jsonls'))
        elseif server.name == 'sumneko_lua' then
            opts = utils.merge(opts, require('neovim.lsp.providers.sumneko_lua'))
        end
