...
local function clear_cache()
    if 0 == vim.fn.delete(vim.fn.stdpath('config') .. '/lua/neovim/compiled.lua') then
        vim.cmd(':LuaCacheClear')
    end
end

function M.post_reload(msg)
    local Logger = require('neovim.utils.logger')
    unload('neovim.utils', true)
    unload('neovim.theme', true)
    unload('neovim.plugins.statusline', true)
...
function M.reload_user_config_sync()
    ...
    clear_cache()
    unload('neovim.config', true)
    unload('neovim.core.pluginsInit', true)
    vim.cmd([[autocmd User PackerCompileDone ++once lua require('neovim.utils').post_reload()]])
    vim.cmd(':PackerSync')
end
    ...
    Job:new(
        {
            ...
            on_exit = function()
                if vim.tbl_isempty(errors) then
                    Logger:log('Updated! Running neovimReloadSync...')
