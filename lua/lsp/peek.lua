...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or lsp.peek.max_height),
      range.start.line + (opts.max_height or lsp.peek.max_height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(
    if_nil(width, lsp.peek.max_width),
    if_nil(height, lsp.peek.max_height),
    opts
  )
