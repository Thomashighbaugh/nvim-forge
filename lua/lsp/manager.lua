...
local function resolve_config(server_name, ...)
  ...
  local has_custom_provider, custom_config = pcall(require, "nvoid/lsp/providers/" .. server_name)
...
function M.setup(server_name, user_config)
  ...
  if nvoid_lsp_utils.is_client_active(server_name) or client_is_configured(server_name) then
    return
  end
  ...
  local should_auto_install = function(name)
    local installer_settings = lsp.installer.setup
